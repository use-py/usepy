# -*- coding: utf-8 -*-
"""
update from https://github.com/mewwts/addict
"""
import copy
...
class AdDict(dict):
    ...
    def __setattr__(self, name, value):
        if hasattr(self.__class__, name):
            raise AttributeError(
                "'AdDict' object attribute " "'{0}' is read-only".format(name)
            )
        else:
            self[name] = value
    ...
    def __setitem__(self, name, value):
        ...
        if is_frozen and name not in super(AdDict, self).keys():
            raise KeyError(name)
        ...
        if (
            ...
            and not isinstance(value, AdDict)
        ):
