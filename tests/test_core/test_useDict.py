from usepy import useAdDict, useDict, useList
...
def test_use_list():
    ul = useList()
    assert list(ul.split([1, 2, 3, 4, 5, 6, 7, 8, 9], 3)) == [
        [1, 2, 3],
        [4, 5, 6],
        [7, 8, 9],
    ]
...
def test_use_dict():
    ...
    assert useDict.arrays_to_dict(["a", "b", "c"], [1, 2, 3]) == {
        "a": 1,
        "b": 2,
        "c": 3,
    }
...
def test_merge():
    assert useDict.merge({"a": 1, "b": 2}, {"c": 3, "d": 4}) == {
        "a": 1,
        "b": 2,
        "c": 3,
        "d": 4,
    ...
    assert useDict.merge_values({"a": 1, "b": 2}, {"a": 3, "b": 4}) == {
        "a": [1, 3],
        "b": [2, 4],
    }

    assert useDict.merge_value({"a": 1, "b": "foo", "c": 400, "d": 400}) == {
        1: ["a"],
        "foo": ["b"],
        400: ["c", "d"],
    }
